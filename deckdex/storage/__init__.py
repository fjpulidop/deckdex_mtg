...
def get_collection_repository(database_url: Optional[str], engine=None) -> Optional[CollectionRepository]:
    ...
    if engine is None and (not database_url or not database_url.strip().startswith("postgresql")):
        return None
    return PostgresCollectionRepository(database_url or "", engine=engine)
