...
class ProcessorConfig:
    ...
    def api_delay(self) -> float:
        ...
        warnings.warn(
            "config.api_delay is deprecated, use config.processing.api_delay instead", DeprecationWarning, stacklevel=2
        )
    ...
    def use_openai(self) -> bool:
        ...
        warnings.warn(
            "config.use_openai is deprecated, use config.openai.enabled instead", DeprecationWarning, stacklevel=2
        )
...
class ClientFactory:
    ...
    def create_spreadsheet_client(config: ProcessorConfig):
        ...
        if config.dry_run:
            ...
            return SpreadsheetClient(credentials_path, config.google_sheets)
