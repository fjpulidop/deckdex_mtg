...
_job_results: Dict[str, Tuple[dict, float]] = {}  # job_id -> (result, completed_at_timestamp)
...
async def list_jobs(user_id: int = Depends(get_current_user_id)):
    ...
    for job_id, service in _active_jobs.items():
        ...
        if service is None:
            # Sentinel entry (e.g. catalog sync) — skip, it has no metadata
            continue
    ...
    for job_id, (result, _completed_at) in _job_results.items():
        in_memory_ids.add(job_id)
    ...
    # Jobs from Postgres not already in memory: running/pending always, plus
    # recently completed (within 2h) so they survive in-memory eviction.
    ...
    if job_repo:
        try:
            ...
            db_jobs = job_repo.get_job_history(user_id, limit=50)
            for db_job in db_jobs:
                jid = str(db_job.get("job_id") or db_job.get("id", ""))
                if not jid or jid in in_memory_ids:
                    continue
                status = db_job.get("status", "")
                # Always include running/pending; include completed only if recent
                if status in ("running", "pending"):
                    include = True
                elif status in ("complete", "error", "cancelled"):
                    completed_str = db_job.get("completed_at")
                    if completed_str:
                        completed_dt = datetime.fromisoformat(str(completed_str))
                        if completed_dt.tzinfo is None:
                            completed_dt = completed_dt.replace(tzinfo=timezone.utc)
                        include = completed_dt >= cutoff
                    else:
                        include = False
                else:
                    include = False
...
async def trigger_process(
    ...
    for job_id, service in _active_jobs.items():
        if service.status == "running" and _job_types.get(job_id) == "process":
            ...
            raise HTTPException(
                status_code=409, detail=f"Another process (full) job is already running (job_id: {job_id})"
            )
    ...
    async def progress_callback(event):
        ...
        if event_type == "progress":
            await ws_manager.send_progress(
                job_id, event.get("current", 0), event.get("total", 0), event.get("percentage", 0.0)
            )
        elif event_type == "error":
            await ws_manager.send_error(
                job_id, event.get("card_name", ""), event.get("error_type", "unknown"), event.get("message", "")
            )
        elif event_type == "complete":
            await ws_manager.send_complete(job_id, event.get("status", "unknown"), event.get("summary", {}))
    ...
    async def run_process():
        try:
            ...
            _job_results[job_id] = (result, time.monotonic())
            if result.get("status") == "success":
                clear_collection_cache()
                clear_stats_cache()
        except Exception as e:
            logger.error(f"Process job {job_id} failed: {e}")
            _job_results[job_id] = ({"status": "error", "error": "Job failed"}, time.monotonic())
    ...
    return JobResponse(job_id=job_id, status="pending", message="Process job created and queued")


@router.post("/prices/update", response_model=JobResponse)
@limiter.limit("5/minute")
async def trigger_price_update(
    request: Request, background_tasks: BackgroundTasks, user_id: int = Depends(get_current_user_id)
):
    """
    ...
    for job_id, service in _active_jobs.items():
        if service.status == "running" and _job_types.get(job_id) == "update_prices":
            ...
            raise HTTPException(
                status_code=409, detail=f"Another price update job is already running (job_id: {job_id})"
            )
    ...
    return JobResponse(job_id=job_id, status="pending", message="Price update job created and queued")
...
async def get_job_status(job_id: str, user_id: int = Depends(get_current_user_id)):
    ...
    if job_id in _job_results:
        result, _completed_at = _job_results[job_id]
        return JobStatus(
            job_id=job_id,
            status=result.get("status", "complete"),
            progress={"summary": result},
...
def _cleanup_old_jobs():
    """Remove completed services from _active_jobs and evict expired results."""
    # Move completed services out of _active_jobs
    completed_ids = [
        jid for jid, svc in _active_jobs.items() if svc is not None and svc.status in ("complete", "error", "cancelled")
    ]
    for jid in completed_ids:
        del _active_jobs[jid]

    # Evict results older than TTL
    now = time.monotonic()
    expired = [jid for jid, (_, completed_at) in _job_results.items() if now - completed_at > _JOB_RESULT_TTL_SECONDS]
    for jid in expired:
        del _job_results[jid]
        _job_types.pop(jid, None)
