...
class ConnectionManager:
    ...
    async def send_progress(self, job_id: str, current: int, total: int, percentage: float, **kwargs):
        ...
        await self.broadcast(
            job_id,
            {"type": "progress", "current": current, "total": total, "percentage": round(percentage, 2), **kwargs},
        )
    ...
    async def send_error(self, job_id: str, card_name: str, error_type: str, message: str):
        ...
        """
        await self.broadcast(
            job_id, {"type": "error", "card_name": card_name, "error_type": error_type, "message": message}
        )
    ...
    async def send_complete(self, job_id: str, status: str, summary: dict):
        ...
        """
        await self.broadcast(job_id, {"type": "complete", "status": status, "summary": summary})
...
async def websocket_progress(websocket: WebSocket, job_id: str):
    ...
    try:
        ...
        if job_id in _active_jobs:
            ...
            result_tuple = _job_results[job_id]
            result_data = result_tuple[0] if isinstance(result_tuple, tuple) else result_tuple
            job_status = result_data.get("status", "complete")

        await websocket.send_json(
            {"type": "connected", "job_id": job_id, "job_status": job_status, "timestamp": datetime.now().isoformat()}
        )
        ...
        if job_status == "running" and (initial_progress.get("total", 0) > 0):
            ...
            raw = _job_results.get(job_id)
            result = raw[0] if isinstance(raw, tuple) else (raw or {})
            await websocket.send_json(
                {"type": "complete", "status": job_status, "summary": result, "timestamp": datetime.now().isoformat()}
            )
