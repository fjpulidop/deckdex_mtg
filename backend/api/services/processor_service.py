...
class ProcessorService:
    ...
    def __init__(
        ...
        self.status = "pending"
        self.progress_data = {"current": 0, "total": 0, "percentage": 0.0, "errors": []}
        ...
        if self.progress_callback:
            try:
                await self.progress_callback(
                    {"type": event_type, "job_id": self.job_id, "timestamp": datetime.now().isoformat(), **data}
                )
            except Exception as e:
    ...
    def _add_error(self, card_name: str, error_message: str):
        ...
        with self._lock:
            self.progress_data["errors"].append(
                {"card_name": card_name, "message": error_message, "timestamp": datetime.now().isoformat()}
            )
    ...
    async def cancel_async(self):
        ...
        await self._emit_progress(
            "complete", {"status": "cancelled", "summary": {"status": "cancelled", "message": "Job cancelled by user"}}
        )
