...
GitHub Actions CI workflow with backend (pytest + ruff) and frontend (lint + typecheck + vitest) jobs. Runs on every push and pull request to `main`. Uses PostgreSQL 15 service container for backend tests and caches pip/npm dependencies for fast runs.
