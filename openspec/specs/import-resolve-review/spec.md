# Import Resolve & Review

Card name resolution with user review before import. Covers the `/api/import/resolve` endpoint and the Review step UI in the import wizard.
...
The backend SHALL accept `POST /api/import/external/cards` with a JSON body containing a `cards` array (each with `name`, `quantity`, optional `set_name`) and `mode` field, as an alternative to file/text upload. This allows the frontend to send user-corrected card names directly.
