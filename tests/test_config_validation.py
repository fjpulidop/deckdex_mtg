...
from deckdex.config import GoogleSheetsConfig, OpenAIConfig, ProcessingConfig, ProcessorConfig, ScryfallConfig
...
class TestProcessingConfig(unittest.TestCase):
    ...
    def test_valid_config(self):
        ...
        config = ProcessingConfig(batch_size=20, max_workers=4, api_delay=0.1, write_buffer_batches=3)
...
class TestScryfallConfig(unittest.TestCase):
    ...
    def test_valid_config(self):
        """Test creating config with valid values."""
        config = ScryfallConfig(max_retries=3, retry_delay=0.5, timeout=10.0)
...
class TestGoogleSheetsConfig(unittest.TestCase):
    ...
    def test_valid_config(self):
        ...
        config = GoogleSheetsConfig(
            batch_size=500, max_retries=5, retry_delay=2.0, sheet_name="magic", worksheet_name="cards"
        )
...
class TestOpenAIConfig(unittest.TestCase):
    ...
    def test_valid_config(self):
        """Test creating config with valid values."""
        config = OpenAIConfig(enabled=True, model="gpt-4", max_tokens=150, temperature=0.7, max_retries=3)
